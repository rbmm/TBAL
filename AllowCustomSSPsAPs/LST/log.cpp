...
#define L_F L"\\systemroot\\temp\\LST\\%u-%02u-%02u.log"

NTSTATUS CLogFile::Init()
{
	HANDLE hFile;
	IO_STATUS_BLOCK iosb;
	UNICODE_STRING ObjectName = { sizeof(L_F) - sizeof(L"\\%u-%02u-%02u.log"), sizeof(L_F), const_cast<PWSTR>(L_F) };
...
NTSTATUS CLogFile::Init(_In_ PTIME_FIELDS tf)
{
	...
	if (0 >= swprintf_s(lpFileName, _countof(lpFileName), L_F, tf->Year, tf->Month, tf->Day))
	{
